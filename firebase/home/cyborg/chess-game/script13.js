...
(() => {
  ...
  async function joinGame(gid) {
    ...
    const data = snap.val();
    if (data?.board) {
      try { chess.load(data.board); } catch {}
    } else {
      chess.reset();
    }
    renderBoard();
  ...
  function leaveGame() {
    // no DB deletions here (host cleanup happens on game over path)
    resetLocalState();
  }
  ...
  function resetLocalState() {
    ...
    engine = null; engineReady = false; engineSearching = false; engineQueue.length = 0;
  ...
  // Chat send
  // --------------------------
  sendChatBtn && sendChatBtn.addEventListener("click", sendChat);
  chatInput && chatInput.addEventListener("keydown", (e) => { if (e.key === "Enter") sendChat(); });

  function sendChat() {
    const txt = (chatInput.value || "").trim();
    if (!txt) return;
    if (!currentGameId) { calert("Chat is only available in multiplayer."); return; }

    const cref = db.ref(`chess/${currentGameId}/chat`);
    cref.transaction((arr) => {
      arr = arr || [];
      arr.push({
        text: txt,
      ...
      return arr;
    }).catch(e => err("chat send", e));

    chatInput.value = "";
  }

  // --------------------------
  // Auth state + Lobby controls
  // --------------------------
...
/* ======== Global helpers for HTML (optional) ======== */
// Expose createGame if you want to call from HTML or console
window.createGame = window.createGame || function () { console.warn("createGame is available after auth"); };
