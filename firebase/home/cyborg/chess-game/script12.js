...
(() => {
  ...
  // ---------- helpers ----------
  function log(...args) { console.log("[script2]", ...args); }
  function err(...args) { console.error("[script2]", ...args); }
  ...
  function ensureLoader() {
    let loader = document.getElementById("stockfish-loader");
    if (!loader) {
      loader = document.createElement("div");
      loader.id = "stockfish-loader";
      loader.innerHTML = `<div class="spinner"></div><p>Loading Stockfish AI...</p>`;
      ...
      css.textContent = `
        ...
        #stockfish-loader .spinner {
          border:6px solid #f3f3f3;border-top:6px solid #ec6090;
          border-radius:50%;width:60px;height:60px;
          animation:spin 1s linear infinite;margin-bottom:15px;
        }
        @keyframes spin { 0% { transform:rotate(0deg);} 100% { transform:rotate(360deg);} }
        #stockfish-loader.show { display:flex; }
        #stockfish-loader.hidden { display:none; }
      `;
      document.head.appendChild(css);
    }
    return loader;
  }
  function showLoader() { const l=ensureLoader(); l.classList.add("show"); l.classList.remove("hidden"); }
  function hideLoader() { const l=document.getElementById("stockfish-loader"); if(l) l.classList.remove("show"); }
  ...
  function initStockfish(path = STOCKFISH_PATH) {
    ...
    showLoader(); // show until we get confirmation
    ...
    engine.onmessage = (ev) => {
      ...
      if (line.startsWith("bestmove")) {
        ...
        hideLoader(); // fallback just in case

        const mv = line.split(" ")[1];
        if (mv && mv !== "(none)" && !currentGameId) {
          try {
            const result = chess.move({ from: mv.slice(0,2), to: mv.slice(2,4), promotion: "q" });
            if (result) {
              renderBoard(chess.board());
              currentTurnSpan.textContent = chess.turn() === "w" ? "White" : "Black";
              checkForEndAndNotify();
            }
          } catch (e) { err("[ENGINE MOVE] Failed", e); }
        }
      }
    };
  ...
  function resetLocalState(){
    ...
    if(engine){try{engine.terminate();}catch{} engine=null; engineReady=false; engineSearching=false; engineQueue.length=0;}
...
function listenForGames() {
  const chessRef = firebase.database().ref('chess');
  ...
  chessRef.on('value', (snapshot) => {
    ...
    if (games) {
      Object.entries(games).forEach(([gameId, gameData]) => {
        ...
        if (!gameData.playerBlack && gameData.playerWhite !== firebase.auth().currentUser.uid && gameData.status === 'waiting') {
        ...
        if ((gameData.playerWhite === firebase.auth().currentUser.uid || gameData.playerBlack === firebase.auth().currentUser.uid) &&
            gameData.status !== 'completed' &&
