/* script17.js
   ...
   Includes:
   (3) Timers with sudden-death countdown (configurable base time)
   ...
   - Only the active player's timer is *ticking*. The other timer remains visible but paused.
*/
...
(function () {
  ...
  // Clock config (sudden death; no increment)
  ...
  // Timers (both visible, only active player counts down)
  ...
  // ---------- Local clock (AI games) ----------
  let localTimeWhiteMs = DEFAULT_BASE_MINUTES * MS_PER_MIN;
  let localTimeBlackMs = DEFAULT_BASE_MINUTES * MS_PER_MIN;
  let localActive = "w";
  let localLastTs = 0;
  ...
  // Update timer boxes (active one ticks, both visible)
  function setActiveTimerDisplay(active) {
    if (!whiteTimerBox || !blackTimerBox) return;
    // Both visible; add a "active" class to the ticking side (CSS can style it brighter)
    whiteTimerBox.classList.toggle("active", active === "w");
    blackTimerBox.classList.toggle("active", active === "b");
  }
  function updateTimerNumbers(wMs, bMs) {
    if (whiteTimerSpan) whiteTimerSpan.textContent = formatMs(wMs);
    if (blackTimerSpan) blackTimerSpan.textContent = formatMs(bMs);
  }
  ...
  function renderBoard() {
    ...
    if (!currentGameId) {
      updateTimerNumbers(localTimeWhiteMs, localTimeBlackMs);
      setActiveTimerDisplay(localActive);
      updateTurnAndStatusText(`| W: ${formatMs(localTimeWhiteMs)} | B: ${formatMs(localTimeBlackMs)}`);
    }
  }
  ...
  function handleGameEnd(endReason = "") {
    ...
    if (currentGameId) {
      ...
      if (winnerColor) updateEloForGame(currentGameId, winnerColor).catch(e => warn("Elo update failed", e));
    }
  ...
  // ---------- Local (AI) clock ----------
  function startLocalClocks(baseMinutes = DEFAULT_BASE_MINUTES) {
    localTimeWhiteMs = baseMinutes * MS_PER_MIN;
    localTimeBlackMs = baseMinutes * MS_PER_MIN;
    localActive = "w";
    localLastTs = Date.now();
    runLocalTick();
  }
  function stopLocalTick() {
    if (localTickHandle) { clearInterval(localTickHandle); localTickHandle = null; }
  }
  function runLocalTick() {
    ...
    localTickHandle = setInterval(() => {
      const now = Date.now();
      const elapsed = now - localLastTs;
      if (localActive === "w") {
        localTimeWhiteMs -= elapsed;
        localLastTs = now;
        if (localTimeWhiteMs <= 0) {
          localTimeWhiteMs = 0;
          updateTimerNumbers(localTimeWhiteMs, localTimeBlackMs);
          updateTurnAndStatusText(`| W: ${formatMs(localTimeWhiteMs)} | B: ${formatMs(localTimeBlackMs)}`);
          handleGameEnd("Time out: White loses.");
          return;
        }
      } else {
        localTimeBlackMs -= elapsed;
        localLastTs = now;
        if (localTimeBlackMs <= 0) {
          localTimeBlackMs = 0;
          updateTimerNumbers(localTimeWhiteMs, localTimeBlackMs);
          updateTurnAndStatusText(`| W: ${formatMs(localTimeWhiteMs)} | B: ${formatMs(localTimeBlackMs)}`);
          handleGameEnd("Time out: Black loses.");
          return;
        }
      }
      updateTimerNumbers(localTimeWhiteMs, localTimeBlackMs);
  ...
  function localApplyClockOnMove() {
    const now = Date.now();
    const elapsed = now - localLastTs;
    if (localActive === "w") localTimeWhiteMs -= elapsed;
    else localTimeBlackMs -= elapsed;

    localActive = (localActive === "w") ? "b" : "w";
    localLastTs = now;

    if (localTimeWhiteMs <= 0) { localTimeWhiteMs = 0; handleGameEnd("Time out: White loses."); return; }
    if (localTimeBlackMs <= 0) { localTimeBlackMs = 0; handleGameEnd("Time out: Black loses."); return; }
    setActiveTimerDisplay(localActive);
  }

  // ---------- Multiplayer clock ----------
  async function multiplayerApplyClockAndPush() {
    if (!currentGameId) return;
    const gref = db.ref(`chess/${currentGameId}`);
    await gref.transaction(g => {
      if (!g) return g;
      const now = Date.now();

      const active = g.active || "w";
      const lastTs = g.lastTurnTs || now;
      const elapsed = Math.max(0, now - lastTs);

      g.timeWhiteMs = g.timeWhiteMs ?? (DEFAULT_BASE_MINUTES * MS_PER_MIN);
      g.timeBlackMs = g.timeBlackMs ?? (DEFAULT_BASE_MINUTES * MS_PER_MIN);

      if (active === "w") g.timeWhiteMs = Math.max(0, g.timeWhiteMs - elapsed);
      else g.timeBlackMs = Math.max(0, g.timeBlackMs - elapsed);
      ...
      if (g.timeWhiteMs <= 0) {
        g.status = "completed";
        g.winner = "black";
        g.completedAt = firebase.database.ServerValue.TIMESTAMP;
      } else if (g.timeBlackMs <= 0) {
        g.status = "completed";
        g.winner = "white";
        g.completedAt = firebase.database.ServerValue.TIMESTAMP;
      } else {
        g.active = (active === "w") ? "b" : "w";
        g.lastTurnTs = now;
        g.updatedAt = firebase.database.ServerValue.TIMESTAMP;
      }
      return g;
    });
  }
  ...
  // Live display
  let mpTickHandle = null;
  function stopMpTick() {
    if (mpTickHandle) { clearInterval(mpTickHandle); mpTickHandle = null; }
  }
  function startMpTick(snapshotGetter) {
    stopMpTick();
    mpTickHandle = setInterval(async () => {
      try {
        const snap = await snapshotGetter();
        const g = snap.val();
        if (!g) return;
        ...
        let w = g.timeWhiteMs ?? (DEFAULT_BASE_MINUTES * MS_PER_MIN);
        let b = g.timeBlackMs ?? (DEFAULT_BASE_MINUTES * MS_PER_MIN);
        if (g.status !== "completed") {
          if (active === "w") w = Math.max(0, w - elapsed);
          else b = Math.max(0, b - elapsed);
        }
        updateTimerNumbers(w, b);
        setActiveTimerDisplay(active);
        updateTurnAndStatusText(`| W: ${formatMs(w)} | B: ${formatMs(b)}`);
      } catch {}
    }, 300);
  }
    ...
    try {
      ...
      ref.on("value", snap => {
        ...
        Object.entries(games).forEach(([gid, g]) => {
          ...
          const base = `${formatMs(g.timeWhiteMs ?? DEFAULT_BASE_MINUTES*MS_PER_MIN)} / ${formatMs(g.timeBlackMs ?? DEFAULT_BASE_MINUTES*MS_PER_MIN)}`;

          li.innerHTML = `<span class="gid">Game: ${gid}</span> <span class="status">(${status})</span> <span class="time">Time: ${base}</span>`;
  ...
  async function createGame() {
    ...
    try {
      ...
      const payload = {
        ...
        // clock fields
        timeWhiteMs: DEFAULT_BASE_MINUTES * MS_PER_MIN,
        timeBlackMs: DEFAULT_BASE_MINUTES * MS_PER_MIN,
        active: "w",
        lastTurnTs: Date.now(),
      };
  ...
  async function joinGame(gid) {
    ...
    try {
      ...
      subscribeToGame(gid);
      startMpTick(() => db.ref(`chess/${gid}`).get());
    } catch (e) {
    ...
    try {
      ...
      const handler = snap => {
        ...
        if (data.status === "completed") {
          const byTime =
            (data.timeWhiteMs <= 0) ? "Time out: White loses." :
            (data.timeBlackMs <= 0) ? "Time out: Black loses." :
            "";
          handleGameEnd(byTime);
        }
      };
  ...
  function resetLocalState() {
    ...
    stopLocalTick();
    stopMpTick();
    localTimeWhiteMs = DEFAULT_BASE_MINUTES * MS_PER_MIN;
    localTimeBlackMs = DEFAULT_BASE_MINUTES * MS_PER_MIN;
    updateTimerNumbers(localTimeWhiteMs, localTimeBlackMs);
    setActiveTimerDisplay("w");
  ...
  function startAIGameRandomFirst() {
    ...
    openDifficultyModal((level) => {
      startLocalClocks(DEFAULT_BASE_MINUTES);
  ...
  auth.onAuthStateChanged(async user => {
    ...
    if (authStatus) {
      if (user) {
        ...
        if (gameLobby) gameLobby.style.display = "block";
        await ensureUserProfile(user.uid, user.email || "", user.isAnonymous);
        listenForGames();
      } else {
